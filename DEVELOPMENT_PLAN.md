...
| 共享存储 | GPFS `/volume/yzhao04`，两台机器挂载同一路径 |
...
```bash
ssh yzhao04-cpu-shanghai
source /volume/yzhao04/workspace/ICS6201/activate_fly.sh
cd /volume/yzhao04/workspace/ICS6201
...
| GPU 机器 conda 环境激活失败 | 确保 conda 路径在共享存储上一致（`/volume/yzhao04/workspace/miniconda3`） |
